...
namespace Inspector.Infra.Utils
{
    ...
    public static class SettingsUtils
    {
        ...
        private const string SETTINGS_DTR_IO_ADDRESS = "DTRIOAddress";
        private const string SETTINGS_PIN_CODE = "PinCode";
        ...
        public static string GetDTR()
        {
            ...
            var dtr = settings.get_GetSetting(SETTINGS_PLEXOR, SETTINGS_DTR).ToString();

            if (dtr.Equals("Low") || dtr.Equals("High")) return dtr;

            var messsage = "Value for Dtr should either be 'High' or 'Low'";
            throw new ArgumentOutOfRangeException(messsage);
        }

        /// <summary>
        /// Gets the DTRIOAddress.
        /// </summary>
        /// <returns>The DTRIOAddress setting.</returns>
        public static string DTRIOAddress
        {
            get
            {
                var settings = new clsSettings();
                var address = settings.get_GetSetting(SETTINGS_PLEXOR, SETTINGS_DTR_IO_ADDRESS).ToString();
        ...
        public static string PinCode
        {
            ...
            {
                var settings = new clsSettings();
                var code = settings.get_GetSetting(SETTINGS_PLEXOR, SETTINGS_PIN_CODE).ToString();
                if (code.ToUpperInvariant().Equals(SETTINGS_RETURN_NO_VALUE))
                {
                    code = "0000";
                }
                return code;
