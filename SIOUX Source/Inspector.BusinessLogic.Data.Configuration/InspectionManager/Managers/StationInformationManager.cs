...
namespace Inspector.BusinessLogic.Data.Configuration.InspectionManager.Managers
{
    ...
    public class StationInformationManager : IStationInformationManager
    {
        ...
        {
            var stationsInfoLoader = new StationInformationLoader();

            foreach (var prs in stationsInfoLoader.PRSEntities)
            {
                InspectionStatus inspectionStatus;

                var prsName = prs.PRSName;
                var prsInspectionStatusEntity = LookupInspectionStatus(prsName, null, m_InspectionStatusInformationLoader.InspectionStatuses);
                ...
                var stationInfo = new StationInformation(prs.Route, prs.PRSCode, prs.PRSName, prs.PRSIdentification, prs.Information, prs.InspectionProcedure, inspectionStatus, prs.InspectionProcedureId, prs.Id);

                foreach (var prsObject in prs.PRSObjects)
                {
                    var objNameDesc = "";
                    var measPointDesc = "";
                    m_InspectionInformationManager.FindObjectNameDescriptionAndMeasurePointDescription(prsObject.ObjectName,prsObject.MeasurePoint,prsObject.FieldNo,out objNameDesc,out measPointDesc);
                    var prsObjectToAdd = new PrsObject(prsObject.ObjectName, objNameDesc, prsObject.ObjectID, prsObject.MeasurePoint, measPointDesc, prsObject.MeasurePointID, prsObject.FieldNo);
                    stationInfo.AddPrsObject(prsObjectToAdd);
                }

                foreach (var gcl in prs.GasControlLines)
                {
                    var gclName = gcl.GasControlLineName;
                    var gclInspectionStatusEntity = LookupInspectionStatus(prsName, gclName, m_InspectionStatusInformationLoader.InspectionStatuses);
                    ...
                    var gclInformation = new GasControlLineInformation(
                        gcl.Id,
                        gcl.GasControlLineName,
                        gcl.PeMin, gcl.PeMax,
                        gcl.VolumeVA, gcl.VolumeVAK,
                        gcl.PaRangeDM.GetDescription(), gcl.PeRangeDM.GetDescription(),
                        gcl.GCLIdentification, gcl.GCLCode, gcl.FSDStart, gcl.InspectionProcedure, inspectionStatus, gcl.InspectionProcedureId, gcl.StartPosition);

                    foreach (var gclObject in gcl.GCLObjects)
                    {
                        ...
                        var objNameDescription = "";
                        var measurePointDescription = "";
                       m_InspectionInformationManager.FindObjectNameDescriptionAndMeasurePointDescription(gclObject.ObjectName,gclObject.MeasurePoint,gclObject.FieldNo,out objNameDescription,out measurePointDescription);

                        var gclObjectToAdd = new GclObject(gclObject.ObjectName,objNameDescription, gclObject.ObjectID, gclObject.MeasurePoint, measurePointDescription, gclObject.MeasurePointID, gclObject.FieldNo, gclBoundary);
