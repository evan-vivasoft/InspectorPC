...
namespace Inspector.BusinessLogic.Data.Configuration.InspectionManager.XmlLoaders
{
    ...
    internal class StationInformationLoader
    {
        ...
        public List<Model.Station.PRSEntity> PRSEntities { get; set; }
        ...
        private static void RemoveGCLsWithoutPRSs(DataSet dataSet)
        {
            ...
            if (foundRows != null)
            {
                ...
                {
                    string searchString = string.Format("PRSName like '{0}' AND PRSIdentification like '{1}'", row.ItemArray[0].ToString().Replace("'", @"''"), row.ItemArray[1].ToString().Replace("'", @"''"));
        ...
        {
            return value.ToEnum<TypeRangeDM>();
        }
        ...
        {
            return value.ToEnum<UnitOfMeasurement>();
        }
