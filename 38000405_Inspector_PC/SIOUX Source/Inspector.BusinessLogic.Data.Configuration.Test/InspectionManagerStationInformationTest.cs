...
namespace Inspector.BusinessLogic.Data.Configuration.Test
{
    ...
    class InspectionManagerStationInformationTest
    {
        ...
        public void Setup()
        {
            var path = Path.GetDirectoryName(typeof(InspectionManagerStationInformationTest).Assembly.Location);
        ...
        public void LookupInspectionProcedureNameByPrsTest()
        {
            ...
            Assert.IsEmpty(inspectionProcedureName);
        }
        ...
        public void LookupGclObjectWithoutGasControlLineNameWithDuplicatesThrowsExceptionTest()
        {
            ...
            try
            {
                File.Move(@"XML\StationInformation.xml", @"XML\StationInformationCorrect.xml");
                File.Move(@"XML\StationInformationDuplicates.xml", @"XML\StationInformation.xml");

                var stationInfoManager = new StationInformationManager();
                const string prsName = "APELDOORN ORDERMOLENWEG 1 | 5 007 230.1";
                const string measurePoint = "00169";
                var objectName = String.Empty;
                const int fieldNumber = 16;

                stationInfoManager.LookupGasControlLineObject(prsName, measurePoint, objectName, fieldNumber);
            ...
            {
                File.Move(@"XML\StationInformation.xml", @"XML\StationInformationDuplicates.xml");
                File.Move(@"XML\StationInformationCorrect.xml", @"XML\StationInformation.xml");
            }
            Assert.IsTrue(exceptionThrown, "Expected an exception of type InspectorLookupException");
        }
        ...
        private static void ValidateFirstStationAndGasControlLineOfXMLFile(StationInformationManager stationInfoManager)
        {
            ...
            Assert.AreEqual(15, firstGasControlLineInfo.GclObjects.Count);

            GclObject firstGclObject = firstGasControlLineInfo.GclObjects[0];
            Assert.AreEqual(String.Empty, firstGclObject.ObjectName);
            Assert.AreEqual("MeasurePointDescription", firstGclObject.MeasurePointDescription);
            Assert.AreEqual("ObjectNameDescription", firstGclObject.ObjectNameDescription);
