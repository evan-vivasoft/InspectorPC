...
namespace Inspector.Hal.Stub
{
    ...
    public class BluetoothHalSequentialStub : IHal
    {
        ...
        public static int CURRENT_IO_STATUS = 5;
        #endregion Constants
        ...
        private void OnMeasurementTimerTick(object state)
        {
            ...
            {
                IList<Measurement> measurements = GenerateMeasurements(MEASUREMENT_FREQUENCY);
        ...
        private IList<Measurement> GenerateMeasurements(int numberOfMeasurements)
        {
            IList<Measurement> measurements = new List<Measurement>();
            for (int i = 0; i < numberOfMeasurements; i++)
            {
                Console.WriteLine("generating measurements, with io status: " + CURRENT_IO_STATUS);
                measurements.Add(new Measurement(m_CurrentMeasurementValue,CURRENT_IO_STATUS));
        ...
        public void SendCommand(DeviceCommand command, string commandParameter)
        {
            ...
            if (m_ReactionStack.Count > 0)
            {
                ...
                var reaction = m_ReactionStack.Pop();
                Console.WriteLine("sending reaction: " + reaction +" messages remaining: " + m_ReactionStack.Count);
            ...
            {
                Console.WriteLine("received sendcommand '"+ command +"' with no reactions remaining");
            }
        }
        ...
        public void StopContinuousMeasurement()
        {
            ...
            {
                Name = "BTHAL Sequential stub: OnContinuousMeasurementStopped"
            };
        ...
        private void OnConnected()
        {
            if (Connected != null)
            {
                Connected(this, new ConnectedEventArgs(DeviceType));
            }
        }
