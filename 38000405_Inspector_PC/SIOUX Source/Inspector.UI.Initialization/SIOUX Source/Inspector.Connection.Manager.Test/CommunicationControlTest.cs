...
namespace Inspector.Connection.Manager.Test
{
    ...
    public class CommunicationControlTest
    {
        ...
        public void TestConnectWithoutClaiming()
        {
            ...
            {
                communicationControl.Connect(new Dictionary<string, string>(), new List<BluetoothDongleInformation>(), ConnectWithoutClaimingCallBack);
            }
        }
        ...
        public void ConnectWithoutClaimingCallBack(bool commandSucceeded, int errorCode, string message)
        {
        ...
        public void TestDisconnectWithoutClaiming()
        {
            ...
            using (CommunicationControl communicationControl = new CommunicationControl())
            {
                communicationControl.Disconnect(DisconnectWithoutClaimingCallBack);
            }
        }
        ...
        /// <param name="errorCode">The error code.</param>
        /// <param name="message">The message.</param>
        public void DisconnectWithoutClaimingCallBack(bool commandSucceeded, int errorCode, string message)
        {
            Assert.False(commandSucceeded);
            Assert.AreEqual(ErrorCodes.COMMUNICATIONCONTROL_NOT_YET_CLAIMED, errorCode);
            Assert.AreEqual("CommunicationControl has not yet been claimed.", message);
        ...
        public void CallSendCommandInIncorrectStateTest()
        {
            ...
            CommunicationControl.SendCommand(DeviceCommand.FlushManometerCache, String.Empty, StateMachineErrorCallBack);
        ...
        public void CallRecoverFromErrorInIncorrectStateTest()
        {
            ...
            Assert.IsTrue(CommunicationControl.StartCommunication());
            m_ManualResetEvent.Reset();
            CommunicationControl.RecoverFromError(StateMachineErrorCallBack);
            m_ManualResetEvent.WaitOne(m_ManualResetEventTimeout);
        ...
        public void CallConnectInIncorrectStateTest()
        {
            ...
            Assert.IsTrue(CommunicationControl.StartCommunication());
            m_ManualResetEvent.Reset();
            CommunicationControl.Connect(new Dictionary<string, string>(), new List<BluetoothDongleInformation>(), AcceptSucceededCallbacked);
            m_ManualResetEvent.WaitOne(m_ManualResetEventTimeout);
            Assert.IsTrue(m_CallBackExecuted);
            ...
            m_CallBackExecuted = false;
            m_ManualResetEvent.Reset();
            CommunicationControl.Connect(new Dictionary<string, string>(), new List<BluetoothDongleInformation>(), StateMachineErrorCallBack);
            m_ManualResetEvent.WaitOne(m_ManualResetEventTimeout);
            Assert.IsTrue(m_CallBackExecuted);
        ...
        public void CallDisconnectInIncorrectStateTest()
        {
            ...
            m_CallBackExecuted = false;
            m_ManualResetEvent.Reset();
            CommunicationControl.Disconnect(StateMachineErrorCallBack);
            m_ManualResetEvent.WaitOne(m_ManualResetEventTimeout);
            Assert.IsTrue(m_CallBackExecuted);
            ...
            m_CallBackExecuted = false;
            m_ManualResetEvent.Reset();
            (((CommunicationControl as CommunicationControl).ConnectionStateMachine as ConnectionStateMachine).Hal as BluetoothHalSequentialStub).GenerateError();
            m_ManualResetEvent.WaitOne(m_ManualResetEventTimeout);
            Assert.IsTrue(m_CallBackExecuted);
        ...
        public void CallContinuousMeasurement()
        {
            ...
            m_CallBackExecuted = false;
            m_ManualResetEvent.Reset();
            CommunicationControl.Disconnect(AcceptSucceededCallbacked);
            m_ManualResetEvent.WaitOne(m_ManualResetEventTimeout);
            Assert.IsTrue(m_CallBackExecuted);
        ...
        public void CallStopContinuousMeasurementWhenNotStartedIsNotAllowed()
        {
            ...
            m_ManualResetEvent.Reset();
            CommunicationControl.StopContinuousMeasurement(StateMachineErrorCallBack);
            m_ManualResetEvent.WaitOne(m_ManualResetEventTimeout);
            Assert.IsTrue(m_CallBackExecuted);
        ...
        public void CallStartContinuousMeasurementWhileAlreadyStartedIsNotAllowed()
        {
            ...
            m_ManualResetEvent.Reset();
            CommunicationControl.StartContinuousMeasurement(measurementFrequency, StateMachineErrorCallBack, null, ProcessMeasurementStartedCallback);
            m_ManualResetEvent.WaitOne(m_ManualResetEventTimeout);
        ...
        /// <param name="errorCode">The error code.</param>
        /// <param name="message">The message.</param>
        public void StateMachineErrorCallBack(bool commandSucceeded, int errorCode, string message)
        {
            Assert.False(commandSucceeded);
        ...
        public void ProcessMeasurementResultCallback(IList<double> measurements)
        {
            ...
            {
                double difference = Math.Abs(measurements[i] - measurements[i - 1]);
        ...
        /// <param name="errorCode">The error code.</param>
        /// <param name="message">The message.</param>
        public void StatemachineNoErrorCallback(bool commandSucceeded, int errorCode, string message)
        {
            Assert.IsTrue(commandSucceeded);
            Assert.AreEqual(0, errorCode);
            m_CallBackExecuted = true;
