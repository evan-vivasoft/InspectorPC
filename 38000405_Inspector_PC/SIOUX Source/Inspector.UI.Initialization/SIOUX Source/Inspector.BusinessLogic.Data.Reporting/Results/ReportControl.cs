...
namespace Inspector.BusinessLogic.Data.Reporting.Results
{
    ...
    public class ReportControl : IReportControl
    {
        ...
        public static InspectionReport ReadResultFile(string resultFile)
        {
            ...
            {
                string error = string.Format(CultureInfo.InvariantCulture, "Could not read xml file. Exception: {0}", exception);
                //LogHelper.Log(cLogger, LogHelper.LogLevel.Error, error);
            ...
            try
            {
                ...
                if (File.Exists(m_TmpResultsPath))
                {
                    ...
                    if (File.Exists(m_ResultsPath))
                    {
                        ...
                        InspectionReport currentInspection = ReadResultFile(m_TmpResultsPath);
                        InspectionReport totalInspections = ReadResultFile(m_ResultsPath);
