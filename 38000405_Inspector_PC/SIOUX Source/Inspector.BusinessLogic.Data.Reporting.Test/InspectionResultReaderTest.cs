...
namespace Inspector.BusinessLogic.Data.Reporting.Test
{
    ...
    public class InspectionResultReaderTest
    {
        ...
        public void Setup()
        {
            var path = Path.GetDirectoryName(typeof(InspectionResultReaderTest).Assembly.Location);
        ...
        [Test]
        public void LookupAllReportsTest()
        {
            var prsName = "Aesch Belreba Kunde";
            var gclName = "Betriebsschiene";
            var inspectionResultReader = new InspectionResultReader();
            var reportInspectionResult = inspectionResultReader.LookupAllResults(prsName, gclName);

            Assert.AreEqual(3, reportInspectionResult.Count, "Should have received 3 reports");
            Assert.AreEqual(2016, DateTime.Parse(reportInspectionResult[0].DateTimeStamp.StartDate).Year);
            Assert.AreEqual(2015, DateTime.Parse(reportInspectionResult[1].DateTimeStamp.StartDate).Year);
            Assert.AreEqual(2014, DateTime.Parse(reportInspectionResult[2].DateTimeStamp.StartDate).Year);
